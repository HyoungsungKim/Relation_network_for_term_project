...
def omniglot_character_folders():
    ...
    '''
    >>> a = [(i,j) for i in range(3) if 1==1 for j in range(3)]
    >>> a
...
class OmniglotTask(object):
    ...
    # * If set num_samples = 20 and character_folders = metatrain_character_folders, we generate tasks for meta training
    # * If set num_samples = 1 or 5 and character_folders = metatest_character_folders, we generate tasks for meta testing
...
class Omniglot(FewShotDataset):
    ...
    def __init__(self, task, split='train', transform=None, target_transform=None):
        super(Omniglot, self).__init__(task, split, transform, target_transform)
        
    def __getitem__(self, idx):
        # print(f'len(self.image_roots) : {len(self.image_roots)} idx : {idx}')
        if idx >= len(self.image_roots):
            idx = random.randrange(0, len(self.image_roots))
...
def get_data_loader(task, num_per_class=1, split='train', shuffle=True, rotation=0):
    # * NOTE: batch size here is # instance PER CLASS
