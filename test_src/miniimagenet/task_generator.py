...
def mini_imagenet_folders():
    ...
    metatrain_folders = [
        os.path.join(train_folder, label)
        for label in os.listdir(train_folder)
        if os.path.isdir(os.path.join(train_folder, label))
    ]
    
    metatest_folders = [
        os.path.join(test_folder, label)
...
def get_mini_imagenet_data_loader(task, num_per_class=1, split='train', shuffle=False, rotation=0):
    normalize = transforms.Normalize(mean=[0.92206, 0.92206, 0.92206], std=[0.08426, 0.08426, 0.08426])
    dataset = MiniImagenet(task, split=split, transform=transforms.Compose([Rotate(rotation), transforms.ToTensor(), normalize]))
